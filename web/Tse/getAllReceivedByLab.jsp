<%-- 
    Document   : getAllReceivedByLab
    Created on : Jan 3, 2018, 2:20:58 PM
    Author     : Manish Jangir
--%>
<html>
    <jsp:include page="../UItemplate/header.jsp">
        <jsp:param name="title" value="Sample(s) Received by Lab"/>
    </jsp:include>
    ...
    <body>
        ...
        <div class="container-fluid">
            <h4 class="text-center text-info" style="text-decoration: underline">${sampleType} Sample(s) received by Lab</h4>
            ...
            <div class="row">
                ...
                <div class="col-1" style="margin-top: 30px">
                    <div class="col-xs-1" style="padding-top: 10px">
                        <button id="filterSearchfrSC" class="btn btn-default btn-success" data-status="2" data-toggle="tooltip" data-placement="bottom" title="Search Records by Product Name">Search By</button>
                    </div>
                </div>
            </div>
            <br>
            <br>
            <table id="receivedByLab" class="table table-bordered table-hover tbCenter" cellspacing="0" width="100%">
                <thead style="font-size: x-small">
                <th style="width: 85px">SAMPLING<br/>TYPE</th>
                <th style="width: 80px;">EDIT<br/> SAMPLE ID</th>
        ...
        <script>
            $(document).ready(function() {
                $(document).on('click','#receivedByLab > tbody > tr',function () {
//                    var id = $('#sentToCustomer > tbody > tr:nth-child(1)').find('td:nth-child(1)').html().trim();
                    ...
                    window.open('${pageContext.request.contextPath}/Tse/GetUpdateSample?smplid='+id+'&sampleType='+sampleType+'&labCode='+labCode+'&status=2&csrftoken=${sessionScope.csrfToken}');
                });
...
<tr style="font-size:small" class="priorityLevel${data.samplepriorityId}">
    <td class="text-center">
        <a href="${pageContext.request.contextPath}/Tse/GetUpdateSample?smplid=${data.sampleId}&status=${data.statusId}" title="Click to edit Sample details sent to Lab"> ${data.sampleId}</a>
    </td>
