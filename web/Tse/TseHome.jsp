...
<div class="container-fluid">
    <div class="row">
        <div class="col">
            <div class="card card-stats">
                <div class="card-header card-header-warning card-header-icon">
                    <a href="${pageContext.request.contextPath}/Tse/GetCreatedSamples?status=0&csrftoken=${sessionScope.csrfToken}" style="color:white">
                        <div class="card-icon">
                ...
                <div class="card-footer">
                    <div class="stats">
                        <c:if test="${stat0 ne 0}">
                            <i class="material-icons text-danger">Alert (Kindly Generate IOM(s))</i>
                        </c:if>
        ...
        <div class="col">
            <div class="card card-stats">
                <div class="card-header card-header-rose card-header-icon">
                    <a href="${pageContext.request.contextPath}/Tse/GetCreatedSamples?status=1&csrftoken=${sessionScope.csrfToken}" style="color:white">
                        <div class="card-icon">
                            <i class="material-icons">${stat1}</i>
                        </div>
                    </a>
                    <p class="card-category">IOM Generated but not received by Lab</p>
        ...
        <div class="col">
            <div class="card card-stats">
                <div class="card-header card-header-success card-header-icon">
                    <a href="${pageContext.request.contextPath}/Tse/GetCreatedSamples?status=2&csrftoken=${sessionScope.csrfToken}" style="color:white">
                        <div class="card-icon">
        ...
        <div class="col">
            <div class="card card-stats">
                <div class="card-header card-header-info card-header-icon">
                    <a href="${pageContext.request.contextPath}/Tse/GetCreatedSamples?status=3&csrftoken=${sessionScope.csrfToken}" style="color:white">
                        <div class="card-icon">
        ...
        <div class="col">
            <div class="card card-stats">
                <div class="card-header card-header-info card-header-icon">
                    <a href="${pageContext.request.contextPath}/Tse/GetCreatedSamples?status=4&csrftoken=${sessionScope.csrfToken}" style="color:white">
                        <div class="card-icon">
...
<script>
    function nooftanksGraph(series, drilldown) {
...
// Build the chart
        ...
        Highcharts.chart('upcomingGraph', {
            ...
            title: {
                text: 'Top 10 Upcoming Data (customer-wise)',
                style: {
                    color: '#fff'
                }
            },
...
// Build the chart
        ...
        Highcharts.chart('last10Graph', {
            ...
            title: {
                text: 'Top 10 Pending Data (customer-wise)',
                style: {
                    color: '#fff'
                }
            },
    ...
    $(document).ready(function () {
        var newArr = [], series = [], drilldown = [];

        $.ajax({
            async: false,
            url: '/ServoCMP/Tse/redirectController?url=getUpcomingData',
            type: 'POST',
            dataType: 'JSON',
        ...
        $.ajax({
            async: false,
            url: '/ServoCMP/Tse/redirectController?url=getLast10Data',
            type: 'POST',
            dataType: 'JSON',
            success: function (data) {
                console.log("drilldownData0");
                data = formatJSONArray1(data);
                var seriesData = _(data)
                        .groupBy('name')
                        .map((objs, key) => ({
                ...
                console.log("drilldownData1");
                Last10DataGraph(seriesData, drilldownData);
                console.log("drilldownData2");
            },
            error: function (error) {
                console.log(error);
            }
        ...
        $.ajax({
            async: false,
            url: '/ServoCMP/getHighchartsData',
            type: 'POST',
            dataType: 'JSON',
            success: function (data) {
                var commonInds = data.reduce(function (res, currentValue) {
                ...
                $.each(commonInds, function (i, v) {
                    var drilldownData = [];
                    newArr = data.filter(a => a.name == v);
                    var total = 0;
                    newArr.filter((a) => total += parseInt(a.y));//total units in particular industry
                    var temp = [];
                    temp = newArr.filter((a) => a.name == v);//group by industry
                    $.each(temp, function (i, v) {
                        drilldownData.push([v.custName, parseInt(v.y)]);
                    });
                    var tempobj = new Object();
                    tempobj.id = v;
                    tempobj.data = drilldownData;
                    var obj = new Object();
                    obj.name = v;
                    obj.y = total;
                    obj.drilldown = v;
                    series.push(obj);
                    drilldown.push(tempobj);
                });
//                console.log('Series');
//                console.log(series);
...
//                console.log(drilldown);
                ...
                nooftanksGraph(series, drilldown);
