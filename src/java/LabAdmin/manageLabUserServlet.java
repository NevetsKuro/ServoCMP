...
public class manageLabUserServlet extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        List<MstUser> mstUser = null;
        List<MstRole> mstRole = MstRoleDAO.listAllLabRoles();
        switch (request.getServletPath()) {
            case "/LabAdmin/AllActiveUser":
                mstUser = MstUserDAO.listAllUser("SELECT MST_USER.EMP_CODE, MST_USER.EMP_NAME, MST_USER.EMP_EMAIL, MST_USER.CNTRLG_EMP_CODE, MST_USER.CNTRLG_EMP_NAME, MST_USER.CNTRLG_EMP_EMAIL, MST_USER.ROLE_ID, MST_USER.UPDATED_BY, MST_USER.UPDATED_DATETIME, MST_ROLE.ROLE_NAME, MST_USER.ACTIVE FROM MST_USER INNER JOIN MST_ROLE ON MST_USER.ROLE_ID = MST_ROLE.ROLE_ID WHERE  MST_USER.ROLE_ID IN (2,4) AND MST_USER.ACTIVE = 1");
                break;
            case "/LabAdmin/AllDeactiveUser":
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (request.getServletPath()) {
            ...
            case "/LabAdmin/ChangeRole":
                ...
                if (mstUser.getMstRole().getRoleId().equals("1") || mstUser.getMstRole().getRoleId().equals("3")) {
                    md.setMsgClass("text-danger");
